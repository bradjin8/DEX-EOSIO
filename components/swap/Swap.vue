          ...
          SelectToken(v-model="outputAmount" :tokens="tokens1" readonly :token="1" @change="tokenChanged(1)")
            template(slot="end")
              .pair(@click="$router.push('/swap/create')").text-muted
                i.el-icon-plus.mr-2
                span Create pool
      //create commission fee part
      .row.mt-3
        .col
          .d-flex.mb-1.select-label.align-items-center
            .item.justify-content-between.w-100
              .top.d-flex.justify-content-between
                h5 {{feeCommission}} % fee tier
                el-button(tag="el-button" size="small" icon="" @click='editFunc' ) {{feeBtnTitle}}
              .bottom.d-flex.justify-content-between
                small.el-tag.el-tag--small.el-tag--dark {{feeValue}}
          .d-flex.mb-1.justify-content-between.flex-row
            //div.col-lg-3(v-for='fee in feeData')
            //  .item(v-if='editShow') {{fee.id}}
          el-radio-group(v-model="feeCommission" size="large" v-if='editShow' @change='changeFee'  ).el-radio-full-width.w-100.mt-2
            el-radio-button.w-25(v-bind:label='fee.value' v-for='fee in feeData' )
              h6 {{fee.value}} %
              small.text-trancate {{fee.status}}
...
export default {
  ...
  data() {
    return {
      ...
      editShow: false,
      feeData: [
        { id: 1, name: 'Best for very stable pairs.', value: 0.01, status: 'Not created' },
        { id: 2, name: 'Best for stable pairs.', value: 0.05, status: '0% select' },
        { id: 3, name: 'Best for most pairs.', value: 0.3, status: '99% select' },
        { id: 4, name: 'Best for exotic.', value: 1, status: 'Best for exotic pairs' }
      ],
      feeCommission: 0.3,
      feeBtnTitle: 'Edit',
      feeValue: '99% select'
  ...
  methods: {
    ...
    editFunc() {
      ...
      this.feeBtnTitle = this.feeBtnTitle === 'Edit' ? 'Hide' : 'Edit'
    },

    changeFee(event) {
      this.feeCommission = event
      this.feeValue = this.feeData.filter(item => item.value === event)[0].status
    }
  }
}
</script>
